...
// GraphQL executes a read-only GraphQL query against GitLab's /api/graphql
// endpoint, decoding the "data" field into out. Query-level errors (e.g.
// insufficient permissions) are returned so collectors can downgrade coverage
// instead of failing.
func (c *Client) GraphQL(ctx context.Context, query string, vars map[string]any, out any) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+graphQLPath, bytes.NewReader(reqBody))
	if err != nil {
		return err
	}
	c.setAuth(req)
	...
	resp, err := c.do(req)
	if err != nil {
		return fmt.Errorf("calling GitLab GraphQL: %w", err)
	}
