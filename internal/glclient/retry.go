...
// do executes a request, transparently waiting out GitLab rate limits up to a
...
// gate proactively pauses before the rate-limit budget is exhausted: once
// remaining drops to the reserve, it waits until the window resets rather than
// firing requests that would be throttled. Concurrent callers all wait out the
// same window; only the first notifies via onWait. After waking, the budget is
// marked unknown so it is relearned from the next response.
...
// response indicates a rate limit at all. GitLab signals throttling with a 429
// plus Retry-After (seconds or HTTP date), and exposes the budget via the
// RateLimit-Remaining / RateLimit-Reset header pair.
func rateLimitWait(resp *http.Response, now time.Time) (time.Duration, bool) {
	...
	if resp.Header.Get("RateLimit-Remaining") == "0" {
		if reset := atoi(resp.Header.Get("RateLimit-Reset")); reset > 0 {
